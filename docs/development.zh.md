...
两次 tsdown 都通过配置的 workspace 路径内各包自有的 `package.json` 发现包；已删除包留下的目录即使仍含 `lib` 或 `node_modules`，也会被忽略。Host 阶段还包含桌面应用。两个阶段都不扫描构建产物来发现 Client 包。包内 tsdown 配置根据 `DSH_BUILD_FACE` 决定当前阶段的入口：普通 Client 插件在 Client 阶段同时生成 Node loader 与 browser bundle；`api-remotes` 通过 `hostPhase: true` 提前生成 Host 入口，再在 Client 阶段只生成 browser bundle。tsdown 只消费 `lib/types` 中由前置 tsc 发射的 JavaScript；仍存在的包若缺少发射入口，构建仍会失败。
